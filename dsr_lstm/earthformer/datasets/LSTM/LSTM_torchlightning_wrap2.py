...
class LSTMLightningDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "test" or stage is None:
            self.lstm_test = LSTM(self.root,train_test_file=self.train_test_file,input_fc=self.input_fc,output_fc=self.output_fc, train=False)

        if stage == "predict" or stage is None:
            self.lstm_predict = LSTM(self.root,train_test_file=self.train_test_file,input_fc=self.input_fc,output_fc=self.output_fc, train=True)
